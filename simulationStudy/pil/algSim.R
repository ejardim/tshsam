...
oms <- mclapply(split(stp, stp), function(x, srModel="ricker"){
	brp <- lh(gislasim(linf=linf, k=k, t0=t0, s=x, a1=a1, a50=a50, ato95=ato95, sl=sl, sr=sr, v=v, a=a, b=b), range=c(min = 0, max = maxage, minfbar = 1, maxfbar = 5, plusgroup = maxage), sr=srModel)
	m(brp)[] <- c(0.8, 0.5, 0.4, 0.3, 0.3, 0.3, 0.3)
	landings.sel(brp)[] <- c(0.05, 0.1, 0.2, 0.3, 0.3, 0.3, 0.1)/0.3
	# by running brp again with different M and Sel, v is recomputed.
	brp <- brp(brp)
	Fmsy <- refpts(brp)["fmax","harvest"]
	Fc <- Fmsy
	...
	ex.sr <- FLSR(model=srModel)
 	params(ex.sr) <- FLPar(abPars(srModel, c(refpts(brp)["virgin","ssb"]/refpts(brp)["virgin","rec"]), v=max(ssb(brp)), s=x))
	stk <- fwd(ex.stk, ctrl=trg, sr=ex.sr, sr.residuals=srr)
	list(brp=brp, stk=window(stk, ny/3, ny))
}, mc.cores=mc)

save(oms, file="RData.oms")
...
# pop split
vps <- as.numeric(dimnames(oms[[1]]$stk@catch.n)$age)
vps[] <- 0.5
...
cat("o ") 
	fmod <- ~te(age, year, k = c(5, 10), bs = "tp")
	qmod <- list(~s(age, k=5))
...
# Had to change the order of diff level to get labels right. 
#====================================================================
attach("RData.sim0")
diffRes$diffusion <- 0
diffResAll <- diffRes
